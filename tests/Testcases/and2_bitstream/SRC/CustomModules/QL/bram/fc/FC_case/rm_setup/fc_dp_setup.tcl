...
## Floorplan initialization variables.  These are used in both the flat and hierarchical DP flows.
set INITIALIZE_FLOORPLAN_UTIL        "0.75" 
set INITIALIZE_FLOORPLAN_CORE_OFFSET "" ;# configure initialize_floorplan with -core_offset; the value specified will be used as X/Y offsets
...
set CREATE_FLOORPLAN_OPERATIONS                "INITIALIZE_FLOORPLAN PLACE_PINS MACRO_PLACEMENT BOUNDARY_CELL TAP_CELL NODE_SUPPLEMENT" ;# List of operations to run during create_power task.
                                                                                                                         ;# See rm_fc_dp_flat_scripts/create_floorplan.tcl for all valid options.
...
set CREATE_POWER_OPERATIONS "PNS STDCELL_PLACEMENT" ;# List of operations to run during create_power task. See rm_fc_dp_flat_scripts/create_power.tcl for all valid options.
...
set PLACE_PINS_OPERATIONS "PLACE_PINS"        ;# List of operations to run during place_pins task. See rm_fc_dp_flat_scripts/place_pins.tcl for all valid options.
...
set DISTRIBUTED 1  ;# Enables distributed processing for block jobs.  Use in conjunction with BLOCK_DIST_JOB_COMMAND and BLOCK_DIST_JOB_FILE (if needed).
...
# 				TOP LEVEL FLOORPLAN CREATION (die, pad, RDL) / PLACE IO
##########################################################################################
set TCL_USER_CREATE_FLOORPLAN_PRE_SCRIPT   "" ;# An optional Tcl file to be sourced at the very beginning of the task
set TCL_USER_CREATE_FLOORPLAN_POST_SCRIPT  "" ;# An optional Tcl file to be sourced at the very end of the task
...
set PLACE_PINS_SELF                        "both" ;# Enables pin placement at top-level design.  This can be performed in different places in the flow.
                                                  ;# The valid options are: create_floorplan, place_pins, none, or both (default).
						  ;# When both is choosen, pins are initially placed in the create_floorplan task.  They are reset prior
...
set CONGESTION_DRIVEN_PLACEMENT       "both" ;# Set to one of the following: std_cell, macro, or both to enable congestion driven placement
...
set PNR_SCRIPTS_DIR "./rm_fc_scripts"		;# Directory of PNR scripts

## Design label and reports variables for the Flat DP flow
set INIT_DESIGN_DP_BLOCK_NAME "init_design_dp"          ;# Label name to be used when saving a block in rm_fc_dp_flat_scripts/init_design_dp.tcl
set COMPILE_DP_BLOCK_NAME "compile_dp"                  ;# Label name to be used when saving a block in rm_fc_dp_flat_scripts/compile_dp.tcl
set CREATE_FLOORPLAN_FLAT_BLOCK_NAME "create_floorplan" ;# Label name to be used when saving a block in rm_fc_dp_flat_scripts/create_floorplan.tcl
set CREATE_POWER_FLAT_BLOCK_NAME "create_power"         ;# Label name to be used when saving a block in rm_fc_dp_flat_scripts/create_power.tcl
set PLACE_PINS_FLAT_BLOCK_NAME "place_pins"             ;# Label name to be used when saving a block in rm_fc_dp_flat_scripts/place_pins.tcl
...
## Design label variables applicable to the FC Hierarchical Synthesis flow
set CREATE_FLOORPLAN_BLOCK_NAME create_floorplan ;# Specify create_floorplan block name
set INIT_COMPILE_BLOCK_NAME init_compile ;# Specify init_compile block name
set TOP_COMPILE_BLOCK_NAME top_compile ;# Specify top_compile block name
