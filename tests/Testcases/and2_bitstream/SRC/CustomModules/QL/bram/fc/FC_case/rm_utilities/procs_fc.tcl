...
proc rm_generate_variables_for_report_parallel { args } {
  ...
  global REPORT_PREFIX REPORT_PARALLEL_SUBMIT_COMMAND REPORTS_DIR REPORT_QOR_REPORT_POWER REPORT_QOR_REPORT_CONGESTION EARLY_DATA_CHECK_POLICY REPORT_POWER_SAIF_FILE REPORT_POWER_SAIF_MAP SAIF_FILE_SOURCE_INSTANCE REPORT_VERBOSE
  ...
  global READ_RTL_BLOCK_NAME COMPILE_BLOCK_NAME COMPILE_INITIAL_OPTO_BLOCK_NAME COMPILE_INCREMENTAL_BLOCK_NAME COMPILE_LOGIC_OPTO_BLOCK_NAME INSERT_DFT_BLOCK_NAME READ_DATA_BLOCK_NAME UNIFIED_FLOW ENABLE_FUSA;# fc_shell version
  ...
  if {[info exists REPORT_PREFIX]} {
    ...
    if {$::synopsys_program_name == "fc_shell"} {
      if {[info exists COMPILE_BLOCK_NAME]} {
        ...
        puts $fid "set COMPILE_INITIAL_OPTO_BLOCK_NAME $COMPILE_INITIAL_OPTO_BLOCK_NAME"
        puts $fid "set COMPILE_INCREMENTAL_BLOCK_NAME $COMPILE_INCREMENTAL_BLOCK_NAME"
        puts $fid "set COMPILE_LOGIC_OPTO_BLOCK_NAME $COMPILE_LOGIC_OPTO_BLOCK_NAME"
    ...
    puts $fid "set ENABLE_FUSA $ENABLE_FUSA"
...
proc auto_multi_vth_constraint {args} {
    ...
    if {[info exists results(-percentage)]} {
        ...
        foreach grp $vt_groups {
            set tmp_vt_groups [sizeof_collection [get_lib_cells -quiet */* -filter threshold_voltage_group==$grp&&dont_use==false&&valid_purposes!=none&&design_type==lib_cell]] ;
            if {$tmp_vt_groups > 0} {
        ...
        if {[llength $open_vt_groups] == 2} {
            ...
            puts "RM-warning: Only one threshold voltage group $open_vt_group is open for optimization. set_multi_vth_constraint will not be applied"
            set PERCENT ""
